...
Promise.all([
    ...
    var nodes = data[0];
    var states = data[1];
    decadeProperties = [1800, 2100, 'visible']
    readyToDraw(nodes, states, decadeProperties)
});

//-----------------------------------------------------------------------------
function readyToDraw(nodes, states, decadeProperties) {
    var colorScale = d3.scaleLinear().domain([-1,5]).range(["#ed6925","#000004","#1b0c41","#4a0c6b","#781c6d","#a52c60","#cf4446"]);
    // var colorScale = d3.scaleOrdinal(d3.schemeCategory10).domain(nodes['name']);
    ...
    nodeLinkG.selectAll('.node-normal')
        ...
        .attr('visibility', function(d) {return (d['year'] >= decadeProperties[0] && d['year'] <= decadeProperties[1]) ? 'visible' : decadeProperties[2]})
...
function onDecadeChanged() {
    ...
    switch(decade) {
        case 'all-time':
            ...
            break;
        case 'before-1979':
            decadeProperties = [1800, 1979,'hidden']
            break;
        case '1980-1989':
            decadeProperties = [1980, 1989,'hidden']
            break;
        case '1990-1999':
            decadeProperties = [1990, 1999,'hidden']
            break;
        case '2000-2009':
            decadeProperties = [2000, 2009,'hidden']
            break;
        case 'after-2010':
            decadeProperties = [2010, 2100,'hidden']
            break;
    ...
    nodeLinkG.selectAll('.node-normal').remove();
    nodeLinkG.selectAll('.node-land').remove();
    myMap.removeLayer(statesLayer);
